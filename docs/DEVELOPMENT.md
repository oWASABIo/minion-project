...
| **@minions/api**    | `apps/api`        | The Backend Engine (Nitro) | `3001` |
| **@minions/shared** | `packages/shared` | Core Logic, Types & Themes | -      |

> [!NOTE]
> As of v1.2.0, most project-related logic (Save, Load, Publish, Analytics) has been centralized into **@minions/api** to ensure thin-client architecture and better security.

### Why Monorepo?

You might ask: _"Why not separate projects?"_
...
The builder uses a centralized design system defined in `packages/shared/src/theme/themes.ts`.

### Adding a New Theme:

1.  Define the palette in `CURATED_THEMES` inside `packages/shared/src/theme/themes.ts`.
2.  Ensure it follows the `ThemeDefinition` interface.
3.  The frontend (`SectionEditor.vue`) will automatically pick up and render the new theme swatch.

### Core Composables:

- **`useThemeVariables`**: Located in `apps/web/composables/`, this parses the system config and injects CSS variables into the page renderer.
